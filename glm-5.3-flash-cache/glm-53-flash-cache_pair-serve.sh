...
#
# External prefix cache (env section 9, PREFIX_CACHE=none|sparkcache|lmcache):
# the launcher builds --kv-transfer-config, mounts PREFIX_CACHE_HOST_PATH at
# /prefix-cache, and for lmcache starts a CPU-only sidecar container
# (<name>-lmcache) before vLLM and waits for its health check. --down and
# --restart stop both; --status lists both.
...
container_down() {
  ...
  # PREFIX_CACHE=lmcache sidecar: stopped AFTER vLLM so in-flight stores and
  # restores drain into a live server. Removed whatever PREFIX_CACHE says now.
...
: "${REPLAYSSM:=}"                     # KK: empty = auto (published: recovery ON for eligible GLM5Next MTP; -188 MiB/GPU, upstream TP4 -3.1% C1); 0 = --no-use-replayssm (JJ-era full-state contract); 1 = --use-replayssm
...
: "${MTP_MOE_BACKEND:=marlin}"          # mtp draft MoE. Original NVFP4 checkpoint has MXFP8 MTP experts: marlin (published KK profile) | humming (JJ default, measured here). b12x is ONLY for the -Spark checkpoint (NVFP4 MTP experts); the engine rejects it for MXFP8 (2026-09-21)
...
# The MTP draft MoE backend depends on the DRAFT experts' format, not the
# target's. Original GLM-5.3-Flash-NVFP4: MXFP8 MTP experts -> marlin or
# humming (b12x boots the target, then dies in the draft constructor:
# "moe_backend='b12x' is not supported for MXFP8 MoE", 2026-09-21). The
# -Spark checkpoint: NVFP4 MTP experts -> b12x (the published TP2 preset).
if [ "$SPECULATOR" = mtp ] && [ "$MTP_MOE_BACKEND" = b12x ]; then
  case "$MODEL_HOST_PATH" in
    *[Ss]park*) : ;;
    *) warn "MTP_MOE_BACKEND=b12x needs NVFP4 MTP experts (the -Spark checkpoint); MODEL_HOST_PATH does not look like one, and the original NVFP4 checkpoint's MXFP8 MTP experts reject b12x at draft load. Use marlin (published) or humming (measured on this pair)" ;;
  esac
fi
...
# suggested value to pin. Remember GLM-5.3-Flash is a hybrid: the aligned
# mamba state cache lives in the same reservation as the paged KV pool, so pin
# the number vLLM reports, not one carried over from an attention-only model.
...
# GB10 is compute capability 12.1. Two GLM-5.3 features on this pin are gated
# to exactly (12, 0) in vllm/models/glm5next/nvidia/:
#   mtp_draft_head.py  -- VLLM_GLM53_MTP_DRAFT_HEAD=nvfp4 (FlashInfer W4A16
#                         CuTe-DSL head copy) raises "requires CUDA capability
#                         12.0; got 12.1" at draft load (measured 2026-09-21).
#                         The GB10 draft head is the older per-checkpoint NVFP4
#                         head (VLLM_MTP_NVFP4_LM_HEAD=1, what Luke's GB10
#                         launcher and our JJ runs use) with DRAFT_HEAD=bf16.
#   l2_prefetch.py     -- VLLM_GLM53_L2_PREFETCH unset = off on 12.1; =1 forces
#                         a 128 MB-L2 (RTX PRO 6000) prefetch policy on. Not
#                         fatal; an A/B, so only reported.
case "${VLLM_GLM53_MTP_DRAFT_HEAD:-bf16}" in
  bf16) : ;;
  nvfp4) die "VLLM_GLM53_MTP_DRAFT_HEAD=nvfp4 requires CUDA capability 12.0 exactly; GB10 is 12.1 and the draft load raises. Set VLLM_GLM53_MTP_DRAFT_HEAD=bf16 and VLLM_MTP_NVFP4_LM_HEAD=1 for the checkpoint's own NVFP4 draft head" ;;
  *) die "VLLM_GLM53_MTP_DRAFT_HEAD must be bf16 or nvfp4: $VLLM_GLM53_MTP_DRAFT_HEAD" ;;
esac
[ "${VLLM_GLM53_L2_PREFETCH:-}" != 1 ] \
  || warn "VLLM_GLM53_L2_PREFETCH=1 forces the RTX PRO 6000 L2 prefetch policy on (engine default on 12.1 is off; Luke's GB10 launcher leaves it unset). Treat as an A/B, not a baseline"
...
# Builds --kv-transfer-config and the extra serve flags, mounts the per-node
# NVMe directory, and (lmcache) describes the CPU-only lmcache server
# container started before vLLM in --run. See env section 9.
prefix_cache_desc="none (GPU prefix cache only)"
lmc_server_cmd=()
...
case "$PREFIX_CACHE" in
  ...
  sparkcache|lmcache)
    ...
    [ "$PREFIX_CACHE_HOST_PATH" != "$CACHE_HOST_PATH" ] || die "PREFIX_CACHE_HOST_PATH must not be CACHE_HOST_PATH (--clear wipes the JIT cache)"
    ...
    # Target checkpoint identity: explicit 64-hex, or auto from the snapshot's
    # config, index and weight-file names+sizes (cheap; identical on both
    # nodes exactly when the snapshots are).
    if [ "$PREFIX_CACHE_MODEL_IDENTITY" = auto ]; then
      pc_target=$(
        {
          printf 'glm53-pair-snapshot-v1\0'
          sha256sum "$MODEL_HOST_PATH/config.json" | cut -d' ' -f1
          if [ -f "$MODEL_HOST_PATH/model.safetensors.index.json" ]; then
            sha256sum "$MODEL_HOST_PATH/model.safetensors.index.json" | cut -d' ' -f1
          fi
          (cd "$MODEL_HOST_PATH" && ls -1 ./*.safetensors | LC_ALL=C sort | xargs stat -L -c '%n %s')
        } | sha256sum | cut -d' ' -f1)
    else
    ...
    pc_layout=$(printf '%s|' "$PREFIX_CACHE" tp2 dcp1 "$BLOCK_SIZE" "$KV_CACHE_DTYPE" \
    ...
    pc_have_image=0
    if command -v "$CONTAINER_RUNTIME" >/dev/null 2>&1 && "$CONTAINER_RUNTIME" image inspect "$SERVING_IMAGE" >/dev/null 2>&1; then
      pc_have_image=1
    fi
    ;;
...
if [ "$PREFIX_CACHE" = sparkcache ]; then
  ...
  prefix_cache_desc="sparkcache -> $PREFIX_CACHE_HOST_PATH (${PREFIX_CACHE_DISK_GB} GiB, spans ${PREFIX_CACHE_SPARK_MIN_SPAN_TOKENS}-${PREFIX_CACHE_SPARK_MAX_SPAN_TOKENS} tokens, ${PREFIX_CACHE_SPARK_ACCESS_MODE})"
fi
...
if [ "$PREFIX_CACHE" = lmcache ]; then
  ...
  prefix_cache_desc="lmcache -> $PREFIX_CACHE_HOST_PATH (L1 ${PREFIX_CACHE_LMC_L1_GB} GiB shm, L2 ${PREFIX_CACHE_DISK_GB} GiB, chunk ${pc_chunk}, servers ${MASTER_ADDR}+${PREFIX_CACHE_LMC_RANK1_ADDR}:${pc_port})"
fi
# ===== END prefix cache =====
