#!/usr/bin/env bash
# qwen38_flash_next_pair_serve.sh -- serve Qwen3.8-Flash-Next-NVFP4 from ONE
# rank of a two-node DGX Spark pair (TP=2 across a direct ConnectX-7 RoCE
# link). Instantiated from template_pair_serve.sh: only the MODEL BLOCK below
# names the model; the body after it is shared pair plumbing.
#
...
# --fairness-engine, --prefill-schedule-interval) exist only on that branch
# and only at certain pins. Every one of them is omitted when its variable is
# empty -- but stock upstream vLLM will reject the B12X backends outright.
#
# Source recipe:
# rtx6kpro/models/qwen38-flash-next/qwen38-flash-next.compose.yml (single-node
# RTX PRO 6000). Per-rank env: qwen38_flash_next-rank.env.example -- copy once
# per node; only section 1 differs.
#
# Design contract (unchanged from the template):
...
#
# Usage:
#     ./qwen38_flash_next_pair_serve.sh --check   rank-0.env   # validate + print command (default)
#     ./qwen38_flash_next_pair_serve.sh --run     rank-0.env   # validate, then start detached
#     ./qwen38_flash_next_pair_serve.sh --restart rank-0.env   # --down, then --run
#     ./qwen38_flash_next_pair_serve.sh --logs    rank-0.env   # follow this rank's container
#     ./qwen38_flash_next_pair_serve.sh --status  rank-0.env   # this rank's container state
#     ./qwen38_flash_next_pair_serve.sh --verify  rank-0.env   # grep the log for startup markers
#     ./qwen38_flash_next_pair_serve.sh --down    rank-0.env   # stop + rm this rank's container
#     ./qwen38_flash_next_pair_serve.sh --clear   rank-0.env   # wipe CACHE_HOST_PATH (guarded)
#     ./qwen38_flash_next_pair_serve.sh --fresh   rank-0.env   # drop page cache, then --run, then follow logs
# Anything after ENV_FILE is appended verbatim to the vllm serve argv.

set -euo pipefail

# MODEL-BLOCK-BEGIN =============================================================
# qwen38_flash_next_pair_serve.sh -- Qwen3.8-Flash-Next-NVFP4 on a two-node DGX Spark
# pair (TP=2 over the direct ConnectX-7 RoCEv2 link).
#
# Source recipe: rtx6kpro/models/qwen38-flash-next/qwen38-flash-next.compose.yml
# (single-node RTX PRO 6000, R35 image localinferencelab/vllm:jovian-judgement-
# community-20260911-r35). Pair adaptations, each commented at its value below:
#   - VLLM_ENABLE_PCIE_ALLREDUCE=0 (TP all-reduce crosses the CX7 link via
#     NCCL/RoCEnante; the launcher refuses 1). The compose's B12X_PCIE_ONESHOT_*
#     knobs are carried but inert with the PCIe path off.
#   - compose NCCL channel tuning (MIN/MAX_NCHANNELS, BUFFSIZE) is single-node:
#     omitted.
#   - compose XDG/TRITON/VLLM/B12X cache roots are dropped: the pair image
#     fingerprints cache roots per revision; CACHE_HOST_PATH mounts /cache.
#   - GPU_MEMORY_UTILIZATION 0.90 (GB10 ceiling ~0.913) instead of 0.96;
#     KV_CACHE_MEMORY_BYTES empty until the pool is measured (bring-up order
#     in the env file tail).
#   - vision ON (LANGUAGE_MODEL_ONLY=0): the checkpoint is multimodal
#     (config.json is_multimodal, qwen3_5_vision tower, image+video tokens).
#     The compose's text-only --language-model-only is NOT carried over.
#
# Status: implemented, NOT qualified. No Qwen TP2/pair serving or performance
# numbers exist yet; the RTX TP1 qualification does not transfer. Measurement
# before publication.
#
...
MODEL_ID='qwen38-flash-next'

# Read-only mount point of MODEL_HOST_PATH inside the container.
MODEL_CONTAINER_PATH='/models/qwen38-flash-next-nvfp4'

# Read-only mount point of DRAFT_MODEL_HOST_PATH; unused: the MTP draft head
# lives in the main checkpoint, so model_needs_draft always returns 1.
DRAFT_CONTAINER_PATH='/models/qwen38-flash-next-nvfp4'

# Legal SPECULATOR values. mtp = the in-checkpoint MTP head (MTP3 qualified on
# RTX TP1); none = plain decode.
SPECULATOR_CHOICES="mtp none"

# fp8 is the qualified layout (B12X QSA attention). The DeepSeek packed
# layouts (fp8_ds_mla / nvfp4_ds_mla) are NOT offered: unverified against QSA.
KV_CACHE_DTYPE_CHOICES="fp8 auto"

# 1: the checkpoint is vision-language. Multimodal block honoured; the env
# file decides text-only vs vision (this launcher ships vision ON).
MODEL_MULTIMODAL=1

# Cosmetic: printed while hashing a draft checkpoint (never happens for mtp).
DRAFT_SIZE_HINT="n/a"

# --verify marker regex. The Qwen-specific terms are best-effort: confirm the
# exact startup log strings from the first real pair boot and tighten here.
# Absence of a marker you expect (a kernel family, a speculator line, the PLE
# line) means a fallback path ran.
VERIFY_LOG_MARKERS='Qwen3.8|qwen3_8|QSA|sparse|indexer|Indexer|GDN|gdn|DeltaNet|PLE|n-gram|ngram|b12x|B12X|B12x|speculative_config|Using .* all-reduce backends|attention block size|Available KV cache memory|GPU KV cache size|Maximum concurrency|Graph capturing finished|cudagraph_mode=|Application startup complete'

# hook: per-model fallbacks. Runs AFTER the env file is sourced, so `: ${X:=}`
# fills only what the file left empty. Sources named per value.
model_defaults() {
  : "${SERVED_MODEL_NAME:=Qwen3.8-Flash-Next}"   # compose --served-model-name
  : "${SPECULATOR:=mtp}"                         # MTP3 is the qualified speculator
  : "${ATTENTION_BACKEND:=B12X}"                 # B12X QSA + indexer (compose omits the flag; B12X is what the engine selects)
  : "${MOE_BACKEND:=b12x}"                       # compose --moe-backend b12x (R33: +11.5% C1 vs FlashInfer CUTLASS on Qwen)
  : "${KV_CACHE_DTYPE:=fp8}"                     # qualified layout for B12X QSA attention
  : "${GPU_MEMORY_UTILIZATION:=0.90}"            # GB10 ceiling ~0.913; profiling fraction while no KV pin is set
  : "${MAX_MODEL_LEN:=262144}"                   # checkpoint max_position_embeddings; compose value
  : "${MAX_NUM_SEQS:=16}"                        # compose value
  : "${MAX_NUM_BATCHED_TOKENS:=6019}"            # compose value; Qwen-measured, never GLM's 4096
  : "${MAX_CUDAGRAPH_CAPTURE_SIZE:=64}"          # compose value; 16 x (3+1) = 64 exact fit
  : "${CUDAGRAPH_MODE:=FULL_AND_PIECEWISE}"      # compose --compilation-config
  : "${LOAD_FORMAT:=instanttensor}"              # compose --load-format; ~2.4 GB/s on Spark NVMe
  : "${TRUST_REMOTE_CODE:=0}"                    # the fork serves this model natively; the compose sets no trust flag
  case "$SPECULATOR" in
    none) : "${NUM_SPECULATIVE_TOKENS:=0}" ;;
    *)    : "${NUM_SPECULATIVE_TOKENS:=3}" ;;    # MTP3: qualified depth (doc: three-token MTP)
  esac
}
...
model_needs_draft() {
  case "$SPECULATOR" in
    *) return 1 ;;   # mtp head is in-checkpoint (config.json mtp.* tensors)
  esac
}

# hook: model-specific validation. Each check cites the source that proves it.
model_validate() {
  # The common body emits --linear-backend from this; the env file must carry it.
  require_value LINEAR_BACKEND
  # Qwen head/PLE/runtime toggles -- all container env from the env file.
  for name in VLLM_PLE_CPU_OFFLOAD VLLM_MXFP8_LM_HEAD VLLM_MTP_NVFP4_LM_HEAD \
              VLLM_LM_HEAD_A16 VLLM_QWEN3_8_FLASH_NEXT_MTP_COMPACT \
              VLLM_QWEN3_8_FLASH_NEXT_OVERLAP VLLM_GDN_SPEC_DECODE_METADATA_FASTPATH \
              VLLM_CAUSAL_CONV1D_UPDATE_HOIST VLLM_USE_V2_MODEL_RUNNER \
              VLLM_COMPUTE_NANS_IN_LOGITS VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS; do
    require_value "$name"; require_bool "$name"
  done
  # rtx6kpro doc (Precision table): 0 produced near-zero MTP draft acceptance
  ...
  # returns to GPU memory and invalidates the KV pin (re-profile).
  [ "$VLLM_PLE_CPU_OFFLOAD" = 1 ] \
    || warn "VLLM_PLE_CPU_OFFLOAD=0 is device-resident PLE: implemented, NOT qualified; re-measure the KV pool"
  if [ "$SPECULATOR" != none ] && [ "$NUM_SPECULATIVE_TOKENS" != 3 ]; then
    warn "MTP3 (3 draft tokens) is the qualified depth: got $NUM_SPECULATIVE_TOKENS"
  fi
  # B12X dynamic MoE knobs carried from the compose recipe (boolean ones).
  for name in B12X_DYNAMIC_SPLIT_ROUTE_COMPUTE B12X_DYNAMIC_DIRECT_EXPERT_SCALES \
              B12X_DYNAMIC_SPLIT_LOW_SMEM B12X_DYNAMIC_SKIP_SPLIT_BARRIER_RESET \
              B12X_DYNAMIC_SPLIT_FAST_PREPARE B12X_DYNAMIC_DETERMINISTIC_OUTPUT \
              B12X_DENSE_SPLITK_TURBO B12X_PCIE_ONESHOT_PDL B12X_MHC_PDL; do
    require_bool "$name"
  done
  [ "$B12X_DYNAMIC_DETERMINISTIC_OUTPUT" = 0 ] \
    || warn "B12X_DYNAMIC_DETERMINISTIC_OUTPUT=1 is off the qualified recipe (runs 0)"
  # R33: requested block 64, measured effective hybrid pages 3008 tokens; the
  # recipe does not force a different geometry. Hybrids override the physical
  # page either way, but 64 is the Qwen-qualified public unit (GLM runs 256).
  [ "$BLOCK_SIZE" = 64 ] \
    || warn "BLOCK_SIZE=$BLOCK_SIZE: the qualified Qwen recipe requests 64 (R33 measured effective hybrid pages 3008 tokens); hybrids override the physical page"
}

# helper for hooks: the adaptive-depth JSON fields (leading comma) or nothing.
# Adaptive depth is verified for GLM's mtp speculator only; Qwen support is
# UNVERIFIED. ADAPTIVE_SPECULATIVE_TOKENS=0 (the default) emits nothing, so
# the risk stays with whoever opts in.
...
# The compose passes moe_backend b12x INSIDE the speculative config (not GLM's
...
# hook: append model-specific vllm serve flags (all from the compose command;
# the common body already emits topology, memory, attention/moe backends,
# load-format, compilation, scheduling, speculative, served-name flags).
model_serve_args() {
  model_args+=(
    --dtype bfloat16
...
# hook: extra `-e KEY=VALUE` container env. The env file carries every Qwen
# variable; nothing needs to beat both --env-file and the image's baked ENV.
model_container_env() {
  :
}

# hook: print the --default-chat-template-kwargs JSON body WITHOUT the braces,
# or nothing. Qwen thinking is controlled per request
# ("chat_template_kwargs":{"enable_thinking":false}); no defaults are forced.
